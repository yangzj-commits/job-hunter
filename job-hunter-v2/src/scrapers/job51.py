"""
求职雷达 · 双轨搜索引擎 (v3.4 - token优化 + 精准度提升)
============================================================
v3.4 改动：
  1. 搜索 Prompt 增加方向限定：只保留数据/运营/咨询/管理等目标方向，
     排除开发/设计/直播/销售等不相关方向
  2. max_tokens 从 4000 降至 2000：搜索结果不需要长回答
  3. 搜索延迟从 2s 降至 1s
  4. 轨道B定向搜索 query 增加方向限定
  5. 自动学习双重优化：
     a) 规则预过滤：公司名含"某"/"未明确"/明显小微特征的直接跳过
     b) 限量：每次最多送 AUTO_LEARN_MAX_CANDIDATES 家去做AI质量判断
  6. 429 快速失败：遇到 TPD 限流立即中止后续搜索，不做无效调用
"""
...
4. 只保留以下目标方向的岗位：
   数据分析、商业分析、信息管理、ERP实施、咨询（管理咨询/风险咨询）、
   产品运营、供应链运营、产品经理、项目管理、审计、IT支持、数字化转型
5. 排除以下方向的岗位（即使搜到也不要返回）：
   软件开发/编程（Java/C++/前端/后端/测试）、UI/平面/视觉设计、
   直播/短视频/主播/拍摄剪辑、安检/保安/保洁、维修/保养、
   销售地推/BD、财务记账/出纳、法务、纯人力资源/招聘岗
...
def _search_and_extract_jobs(query: str, delay: float = 1.0) -> dict:
    ...
    v3.4：delay 从2s降至1s，max_tokens 从4000降至2000
    遇到 TPD 429 抛出 TPDExhaustedError，由调用方决定是否中止。
    """
    time.sleep(delay)
    client = _get_client()
...
# 公司名中出现这些关键词的，直接跳过AI质量判断
GARBAGE_COMPANY_KEYWORDS = [
    "某", "未明确", "未知", "匿名", "信息未",
]

# 公司名中出现这些词缀的小微企业，直接跳过
...
def _update_whitelist_with_new_companies(jobs: list, whitelist: list, config: dict) -> list:
    ...
    # v3.4 优化2：限量，最多送 max_candidates 家去AI判断
...
def fetch_all_jobs(config: dict, whitelist: list) -> tuple:
    all_jobs = []
    seen_hashes = set()
    tpd_exhausted = False  # 429 快速失败标记
    ...
    for kw in keywords:
        ...
        query = (
            ...
            f"只返回数据分析/运营/咨询/管理/审计/ERP方向的岗位，"
            f"不要返回开发编程、设计、直播短视频、安检保安、维修保养类岗位。"
        )
        print(f"  ▸ {kw}")
    ...
    for company in whitelist:
        ...
        query = (
            ...
            f"面向2026届/2027届应届生，"
            f"只找数据分析、运营、咨询、信息管理、ERP、项目管理、审计方向的实习，"
            f"不要返回软件开发、设计、直播短视频、安检保安类岗位。"
        )
        print(f"  ▸ {name}")
