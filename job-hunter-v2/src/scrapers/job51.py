"""
求职雷达 · 双轨搜索引擎 (v3.1 - 修复thinking冲突)
============================================================
v3.1 修复：
  - kimi-k2.5 默认开启 thinking 模式，与 $web_search 不兼容
  - 所有使用 $web_search 的调用加入 extra_body={"thinking": {"type": "disabled"}}
  - 公司质量判断（普通调用）保持默认即可，不受影响
"""
...
# 使用 $web_search 时必须禁用 thinking 模式
...
def _search_and_extract_jobs(query: str, delay: float = 3.0) -> dict:
    ...
    关键：$web_search 调用必须禁用 thinking 模式。
    """
    time.sleep(delay)
    client = _get_client()
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            tools=SEARCH_TOOLS,
            max_tokens=8000,
            extra_body=THINKING_DISABLED,   # ← 关键修复：禁用 thinking
        )
        ...
        for turn in range(5):
            response2 = client.chat.completions.create(
                ...
                tools=SEARCH_TOOLS,
                max_tokens=8000,
                extra_body=THINKING_DISABLED,   # ← 每轮都要禁用
            )
